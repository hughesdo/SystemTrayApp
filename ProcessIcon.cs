        ...
        private bool DownloadYtDlp()
        {
            try
            {
                ...
                using (WebClient client = new WebClient())
                {
                    ...
                    // Download synchronously (we're already on UI thread)
                    client.DownloadFile(downloadUrl, targetPath);
                }

                LogMessage($"Download complete. File size: {new FileInfo(targetPath).Length} bytes");

                // Verify the file exists and has reasonable size (should be several MB)
                if (File.Exists(targetPath) && new FileInfo(targetPath).Length > 1000000)
                {
                    LogMessage("yt-dlp.exe downloaded and verified successfully");
                    return true;
                }
                else
                {
